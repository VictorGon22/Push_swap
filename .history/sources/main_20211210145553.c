...
void algorithm_sort2(t_node *l_a, t_node *l_b, t_variables *var)
{
    ...
    if (check_sorted(var->argc, l_a) == 1)
        ...
        printlinked(l_b);
        if (check_sorted(var->count_la, l_a) == 1)
            printf("ORDENAT LLISTA A:");
        if (check_sorted_inverted(var->count_lb, l_b) == 1)
            printf("ORDENADA LLISTA B");
    }
}
