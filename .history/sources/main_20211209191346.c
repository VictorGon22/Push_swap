...
{
    t_node *tmp_a = l_a;
    int biggernumber = tmp_a->data;
    
    while (tmp_a != NULL)
    {
        if (biggernumber < tmp_a->data) 
            biggernumber = tmp_a->data;          
        tmp_a = tmp_a->nextPtr;
    }
    return (biggernumber);
}
...
void algorithm_sort(t_node *l_a, t_node *l_b, t_variables *var)
{
    t_node *tmp_a = l_a;
    int counteriguals = 0;
    int biggestnumber = find_biggest_nbr(l_a);
    if (check_sorted(var->argc, l_a) == 1)
        printf("ORDENAT");
    else {
        printf("biggestnbr%d", biggestnumber);
        //while (check_sorted(var->argc, l_a) != 1 && var->count_lb == 0) {
        //    
