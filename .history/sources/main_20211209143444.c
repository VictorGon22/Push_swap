...
*/
void adding_structures( int data, t_node *list)
{
    t_node *first_list;
    first_list = malloc(sizeof(t_node));
    first_list->data = data;
    //first_list->nextPtr = list;
    //return (first_list);
}

void my_params_to_list(int ac, char **av, t_node *list)
{
    int i = 0;
    //t_node *list;
    list = NULL;
    ...
    while (i < ac) {
        int num = my_getnbr(av[i]);
        list->nextPtr = list->nextPtradding_structures(num, list);
        list = 
        i++;
    }
    //return (list);
...
int main(int argc, char **argv)
{
    ...
    var.count_la = argc - 1;
    var.count_lb = 0;
    my_params_to_list(argc, argv, l_a);
    //save(argc, argv, l_a);
