...
void algorithm_sort(t_node *l_a, t_node *l_b, t_variables *var)
{
    t_node *tmp_a = l_a;
    int i = 0;
    var->mid_value = find_biggest_nbr(l_a) / 2;
...
void algorithm_sort2(t_node *l_a, t_node *l_b, t_variables *var)
{
    ...
    if (check_sorted(var->argc, l_a) == 1) 
        ...
        //while (check_sorted(var->count_la, l_a) != 1) {
        //    printlinked(l_a);
        //    if (tmp_a->data > var->mid_value) {
        //        func_sa(l_a, var);
        //    }
        ...
        while (check_sorted_inverted(var->count_lb, l_b) != 1) {
            printlinked(l_b);
            if (tmp_a->data > var->mid_value) {
                func_sa(l_b, var);
            }
        ...
        //
        i = 0;
        while (i <= var->count_lb) {
            func_pa(&l_a, &l_b, var);
            i++;
        }
        func_pa(&l_a, &l_b, var);
        printlinked(l_b);
        printlinked(l_a);https://prod.liveshare.vsengsaas.visualstudio.com/join?782F1E687C70036F44D3972B601BF9D5FB92
    }
}
