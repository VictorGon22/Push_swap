...
void createLinkedList(int argc, char **argv, t_node *l_a)
{
    t_node *newNode;
    t_node *currentNodes;
    int nodeData;
    ...
    l_a = (t_node *) malloc(sizeof(t_node));
    if (l_a == NULL) {
        printf("memory cant be allocated");
    } else {
        l_a->data = num = my_getnbr(argv[nodeCtr]);;
        l_a->nextPtr = NULL;
        currentNodes = l_a;
        
        for (nodeCtr = 2; nodeCtr <= argc; nodeCtr++) {
            newNode = (t_node *)malloc(sizeof(t_node));
            if (newNode == NULL) {
                printf("Memory can not be allocated");
                exit (84);
            } else {
                num = my_getnbr(argv[nodeCtr]);
...
int main(int argc, char **argv)
{
    ...
    //save(argc, argv, l_a);
    createLinkedList(argc, argv, l_a);
